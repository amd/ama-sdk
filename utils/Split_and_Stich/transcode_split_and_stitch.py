...
# Usage: python transcode_split_and_stitch.py [options]
...
def check_system_utilization():
    ...
    rc = subprocess.run("print_ma35_load", shell = True, stderr=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)
    
    # Extract the line containing the device utilization
    inUse = False
    out = str(rc.stdout)
...
def progress_meter(percent_donePrev, frames_in_clip, num_segments, running=True):
    if running==False:
        ...
        output = subprocess.Popen("tail -1 amastdout0000.log", shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE, universal_newlines=True).stdout.read()
        outputS = str(output)
...
def get_number_of_devices():
    p = subprocess.run("mautil examine -d all | grep -c MA35D", shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE, universal_newlines=True)
    if (p.returncode!=0):
        print ("Can't determine number of MA35D cards in the system, exiting ...")
        raise SystemExit(-1)
    num_devices = int(str(p.stdout))
    if (num_devices<1):
        print ("No MA35D cards found in the system, exiting ...")
        raise SystemExit(-1)
    return num_devices    
...
def get_max_segments_per_device(width, height, input_format, output_format, framerate_num, framerate_den, num_devices_present):
    ...
    jsonFile = open("./ma35jobdescription.json", "w")
    jsonFile.write(jsonString)
    jsonFile.close()

    # Launch job slot reservation tool
    sp = subprocess.Popen("jobslot_reservation ./ma35jobdescription.json", shell = True, stdin = subprocess.PIPE, stdout = subprocess.PIPE, stderr = subprocess.PIPE)

    # Termine the job slot reservation session by sending ENTER to the program
    output = sp.communicate(input='\n'.encode())[0]

    # Extract the line containing the job slots allocated
    out = output.decode()
    allocated_slots_line = next(line for line in out.splitlines() if "Job Slots Alloted:" in line)

    # Extract the job slots value from the line
    job_slots_allocated = int(allocated_slots_line.split(":")[-1].strip())

    # Determine how many job can be run per device
    max_tot_segments = job_slots_allocated
...
def main():
    ...
    if check_system_utilization() < 0:
        print ("Some jobs are already running and using MA35D resources. Make sure all job finish before running this script.")
        return -1
    ...
    if output_format == "h265":
            output_format = "hevc"
    if output_format != "hevc" and output_format != "h264" and output_format != "av1":
        print ("Output format needs to be h264, h265, hevc or av1")
        raise SystemExit(-1)
    ...
    if input_format == "h265":
        input_format = "hevc"
    if input_format != "hevc" and input_format != "h264" and input_format != "av1":
        print ("Input encoder needs to be h264, h265, hevc or av1")
        raise SystemExit(-1)
    ...
    if num_segments_attempted > 1:
        ...
        split_cmd = "ffmpeg -nostdin -loglevel info  -vsync 0 -i " + input_file + " -c copy -f segment -segment_time " \
            + str(max_segment_duration) + " -y amatmpfile%04d" + input_file[-4:] + " > amastdout.log 2>&1 \n"
    else:
        split_cmd = "cp " + input_file + " amatmpfile00." + input_file[-3:] 
    subprocess.run(split_cmd, shell = True)

    # Check how many segments were created
    proc = subprocess.run("ls amatmpfile* | wc -l", shell = True, stdout = subprocess.PIPE, universal_newlines=True)
    ...
    for n in range(0, num_segments):        
        ...
        inputfile  = "amatmpfile" + format(n, '04d') + input_file[-4:]
        outputfile = "amatmpfileout" + format(n, '04d') + output_file[-4:]
        logfile    = "amastdout" +format(n, '04d')+".log"
        transcode_cmd = "ffmpeg -nostdin -loglevel info -hwaccel ama -hwaccel_device /dev/ama_transcoder"+ str(device_id)+" -vsync 0" + \
                        " -c:v " + input_format + "_ama -i " + inputfile + \
                        " -b:v " + str(bitrate) + "M " + encode_options + \
                        " -c:v " + output_format + "_ama -y " + outputfile + \
                        " > " + logfile + " 2>&1 "
    ...
    for proc in procList:
        if proc.returncode != 0:
            proc = subprocess.run("grep -l \"allocate\" amastdout*.log | wc -l", shell = True, stdout = subprocess.PIPE, universal_newlines=True)
    ...
    output = subprocess.run("printf \"file '%s'\\n\" amatmpfileout* | sort -t _ -k 2 -g > amamylist.txt", shell = True)
    output = subprocess.run("rm -f " + output_file, shell = True)    
    output = subprocess.run("ffmpeg -f concat -safe 0 -i amamylist.txt -c copy " + output_file + " > amastdout.log 2>&1", shell = True)
    endSec = time.time()
...
def cleanup():
    time.sleep(1)
    subprocess.run("rm -f amatmpfile* 2> /dev/null", shell = True)
    subprocess.run("rm -f amamylist.txt 2> /dev/null", shell = True)
    subprocess.run("rm -f amastdout*.log 2> /dev/null", shell = True)
    subprocess.run("rm -f ma35jobdescription.json", shell = True)
