 ...
 *   -o <format> Output pixel format (yuv420p, nv12) [default: yuv420p]
...
static void print_usage(const char *prog_name)
{
    ...
    printf("  -o <format> Output pixel format (yuv420p, nv12) [default: yuv420p]\n");
...
static int init_filters(AppContext *ctx)
{
    ...
    if (strcmp(ctx->output_format, "nv12") == 0)
    ...
    if (ctx->scale_size) {
        /* Use scaler_ama with specified output size, then hwdownload_ama */
        snprintf(filter_desc, sizeof(filter_desc),
                 "scaler_ama=outputs=1:out_res=(%s|%s),hwdownload_ama",
                 ctx->scale_size, ctx->output_format);
    } else {
        /* Direct hwdownload_ama */
        snprintf(filter_desc, sizeof(filter_desc), "hwdownload_ama");
    }
