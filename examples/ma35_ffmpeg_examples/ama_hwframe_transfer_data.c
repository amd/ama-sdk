/*
 * AMD AMA Hardware Frame Transfer Example
 *
 * This example demonstrates how to use the hwdownload_ama filter to transfer
 * decoded frames from hardware (VPE format) to software memory (NV12 format)
 * within a filter graph pipeline.
 *
 * Key concepts demonstrated:
 * - Hardware device initialization for AMD AMA
 * - Hardware-accelerated H.264 decoding with linear format output
 * - Frame transfer using hwdownload_ama filter
 ...
 *
 * Example:
 *   ./ama_hwframe_transfer_data -d info input.mp4
 *
 * Requirements:
...
static int init_filters(AppContext *ctx, const char *filter_description);
...
static void print_usage(const char *prog_name)
{
    printf("AMD AMA Hardware Frame Transfer Example\n\n");
...
static int parse_arguments(int argc, char **argv, AppContext *ctx)
{
    ...
    while ((opt = getopt(argc, argv, "d:o:D:h")) != -1) {
        switch (opt) {
            case 'd':
...
/* Initialize hardware decoder with linear format output */
static int init_decoder(AppContext *ctx)
{
    int ret;
    ...
    /* IMPORTANT: Specify output format to avoid tiled format */
    ...
    av_log(NULL, AV_LOG_INFO, "Decoder initialized: %dx%d (linear format output)\n", 
           ctx->dec_ctx->width, ctx->dec_ctx->height);
    
    return 0;
}

/* Initialize filter graph with hwdownload_ama */
static int init_filters(AppContext *ctx, const char *filter_description)
{
    char args[512];
    int ret = 0;
    ...
    av_log(NULL, AV_LOG_INFO, "Initializing filter graph: %s\n", filter_description);
    ...
    av_log(NULL, AV_LOG_DEBUG, "Buffer source args: %s\n", args);
    ...
    /* Build hw_frames_ctx from hw_device_ctx (dec_ctx->hw_frames_ctx is not
     * populated until the first decoded frame, so we construct it directly). */
    {
        AVBufferRef *hw_frames_ref = av_hwframe_ctx_alloc(ctx->hw_device_ctx);
        if (!hw_frames_ref) { ret = AVERROR(ENOMEM); goto end; }
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Cannot init hw_frames_ctx: %s", av_err2str(ret));
            av_buffer_unref(&hw_frames_ref);
            goto end;
        }
        AVBufferSrcParameters *par = av_buffersrc_parameters_alloc();
        if (!par) { av_buffer_unref(&hw_frames_ref); ret = AVERROR(ENOMEM); goto end; }
        ...
        av_free(par);
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Cannot set buffersrc parameters: %s", av_err2str(ret));
            goto end;
        }
    }

    /* Create unconstrained buffer sink; format is enforced by a "format"
     * filter appended to the chain -- buffersink pix_fmts is deprecated. */
    ...
    if ((ret = avfilter_graph_parse_ptr(ctx->filter_graph, filter_description,
                                         &inputs, &outputs, NULL)) < 0) {
        av_log(NULL, AV_LOG_ERROR, "Cannot parse filter graph: %s\n", av_err2str(ret));
        goto end;
    }
    
    /* Set hardware device context for all filters in the graph */
    if (ctx->hw_device_ctx) {
        for (unsigned int i = 0; i < ctx->filter_graph->nb_filters; i++) {
            ...
            if (!ctx->filter_graph->filters[i]->hw_device_ctx) {
                av_log(NULL, AV_LOG_ERROR, "Cannot set hw_device_ctx for filter %s\n",
                       ctx->filter_graph->filters[i]->name);
                ret = AVERROR(ENOMEM);
                goto end;
            }
        }
...
static int filter_frame(AppContext *ctx, AVFrame *frame)
{
    ...
    while (1) {
        ...
        av_log(NULL, AV_LOG_INFO,
               "Frame %ld transferred to SW: %dx%d format=%s pts=%ld\n",
               ctx->frames_transferred,
               filtered_frame->width, filtered_frame->height,
               av_get_pix_fmt_name(filtered_frame->format),
               filtered_frame->pts);
        
        /* Here you could process the software frame further */
        /* For example: save to file, encode, display, etc. */
...
static int decode_packet(AppContext *ctx, AVPacket *packet)
{
    ...
    while (ret >= 0) {
        ...
        /* Transfer from HW to SW using filter graph */
        if (filter_frame(ctx, frame) < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error filtering frame\n");
        }
        
        /* Properly unreference the hardware frame */
        unref_frame(frame);
    }
...
int main(int argc, char **argv)
{
    ...
    /* scaler_ama is required before hwdownload_ama to prepare the VPI memory
     * state for DMA transfer; raw decoder output frames cannot be downloaded
     * directly. Use a 1:1 scaler pass when no resize is needed. */
    char filter_desc[256];
    snprintf(filter_desc, sizeof(filter_desc),
             "scaler_ama=outputs=1:out_res=(%dx%d|nv12),hwdownload_ama,format=%s",
             ctx.dec_ctx->width, ctx.dec_ctx->height, ctx.output_format);
    if ((ret = init_filters(&ctx, filter_desc)) < 0) {
    ...
    while (av_read_frame(ctx.fmt_ctx, &packet) >= 0) {
        ...
        if (packet.stream_index == ctx.video_stream_index && packet.size > 0) {
            av_log(NULL, AV_LOG_DEBUG, "Processing packet pts=%ld size=%d\n",
    ...
    av_log(NULL, AV_LOG_INFO, "Frames transferred to software: %ld\n", ctx.frames_transferred);
