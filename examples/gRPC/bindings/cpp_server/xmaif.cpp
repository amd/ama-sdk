...
class DownloadImpl final : public Download::Service {
 public:
  ...
  Status Proc(ServerContext* context, const DownloadIn* download_in, DownloadOut *download_out) override {
      ...
      std::string ptr_out_frame_host_p0(reinterpret_cast<char *>(downloadConf.out_frame->data[0].host), downloadConf.frame_props.linesize[0]*downloadConf.frame_props.height);
      std::string ptr_out_frame_host_p1(reinterpret_cast<char *>(downloadConf.out_frame->data[1].host), downloadConf.frame_props.linesize[1]*(downloadConf.frame_props.height >> 1 ));
      std::string ptr_out_frame_host_p2(reinterpret_cast<char *>(downloadConf.out_frame->data[2].host), downloadConf.frame_props.linesize[2]*(downloadConf.frame_props.height >> 1 ));
 ...
 public:
  ...
  Status Proc(ServerContext* context, const UploadIn *upload_in, UploadOut *upload_out) override {
      ...
      if ( upload_in->flush() ) {
          ...
          // The client sends Y/U/V as one contiguous buffer in p0. The upload
          // path (InternalDmaFilterSession::CopyToDevice) DMAs all planes from
          // data[0] as a single block, so the plane pointers must be offsets
          // into that one buffer laid out exactly like xma_frame_alloc:
          //   plane1 at linesize[0]*height, plane2 after plane1's chroma rows.
          // Chroma height is ceil(height/2) (H_ALIGN is not applied for 420P).
          uint8_t* base = reinterpret_cast<uint8_t*>(const_cast<UploadIn *>(upload_in)->mutable_ptr_out_frame_host_p0()->data());
          int32_t chroma_h = (height + 1) >> 1;
          data.data[0] = base;
          data.data[1] = base + uploadConf.frame_props.linesize[0] * height;
          data.data[2] = data.data[1] + uploadConf.frame_props.linesize[1] * chroma_h;
