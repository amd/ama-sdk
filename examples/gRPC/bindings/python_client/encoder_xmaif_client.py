...
def readFullRow(inFile, w):
    row = bytearray()
    while len(row) < w:
        chunk = os.read(inFile, w - len(row))
        if not chunk:
            break
        row += chunk
    return bytes(row)

def frameGeometry(lineSize, height):
    chroma_h = (height + 1) // 2
...
def readIn(inFile, frame_buf, offs, lineSize, width, height, chroma_h):
    bytesRead = 0

    w = width
    dst = offs[0]
    for h in range(height):
        row = readFullRow(inFile, w)
        if len(row) < w:
            return 0
...
def run(IF_IP):
    ...
    with grpc.insecure_channel(f'{IF_IP}:50051') as devServer, \
        ...
        chroma_h, sizes, offs = frameGeometry(lineSize, uplArg.height)
        frame_buf = bytearray(sizes[0] + sizes[1] + sizes[2])
        inFileSize = os.stat(InFile).st_size
        dataread = 0

        while True:

            n = readIn(inFile, frame_buf, offs, lineSize, uplArg.width, uplArg.height, chroma_h)
            if n == 0:
                break
            dataread += n
            ...
            # Send Y/U/V as one contiguous buffer in p0 so the server's
            # single-DMA upload path reads all planes from one base pointer.
            uplIn.ptr_out_frame_host_p0 = bytes(frame_buf)
